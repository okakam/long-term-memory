...
const report = Object.fromEntries(subsets.map((subset) => {
  const queries = gold.queries.filter((query) => query.subset === subset);
  const scores = queries.map((query) => {
    const bm25 = service.searchFulltext(gold.project_id, query.query, { limit: 5 }).map((memory) => memory.name);
    const hybrid = service.searchAssociative(gold.project_id, query.query, { queryEntities: query.query_entities, limit: 5 }).map((memory) => memory.name);
    return {
      id: query.id,
