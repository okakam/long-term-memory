...
- Cloud Run用S3 Markdown adapter、Firestore metadata/auth store、`/tmp` SQLite cache、Firebase ID token/session cookie、API認可、Cloud Run workflow/smoke、認証必須のimage既定値、Firestore metadataを含む双方向移行export/import/verifyを実装済み。
