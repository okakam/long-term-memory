...
- Produces: `DcrClientRegistrationInput = { clientName: string | null; redirectUris: [string]; grantTypes: ['authorization_code', 'refresh_token']; responseTypes: ['code']; tokenEndpointAuthMethod: 'none' }`と`DcrClientRegistrationSchema`。schemaは1件の`redirect_uris`を必須にし、未指定のgrant/response/auth methodを許可値へ正規化してunknown fieldを保存しない。
