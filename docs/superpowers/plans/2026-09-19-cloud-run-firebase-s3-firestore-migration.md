...
- `VerificationReport`はsource/target count、missing key、extra key、hash mismatch、parse failure、membership mismatch、PAT mismatch、tombstone mismatchを含む。
...
- [ ] **手順3: 分離provider smokeを実行する。**
  ...
  専用smoke projectとFirebase user/PATを作成し、Cloud Run候補へ`scripts/cloud-run-smoke.ts`を実行する。initialize、16 tool、save/search/get/update/rename/link/reindex/delete、shared read/write gate、cold start、redacted logを確認する。
