...
初期DCRは、portなしの`http://127.0.0.1/<callback-path>`だけを登録可能にする。authorize時はこの登録値に対してscheme、host、path、queryを完全一致で検証し、`127.0.0.1`のportだけをCodexのlistener用に可変とする。`localhost`、`[::1]`、HTTPS、custom scheme、既にportを持つURIは初期DCRでは拒否する。これはCodexがportなしの`127.0.0.1` callbackだけにlistener portを挿入する仕様に合わせる。[OpenAI公式MCPドキュメント](https://developers.openai.com/codex/mcp/)
...
- DCRはpublic native clientを対象にする。requestは正確に1件の`redirect_uris`、任意`client_name`、任意`grant_types`、任意`response_types`、任意`token_endpoint_auth_method`だけを読み、未指定値は`['authorization_code', 'refresh_token']`、`['code']`、`'none'`へ正規化する。これ以外のgrant/response/auth methodは`invalid_client_metadata`で拒否し、metadataの未知fieldは保存しない。redirect URIはportなしの`http://127.0.0.1` callbackだけを受け付け、wildcard、custom scheme、fragment、`localhost`、IPv6、HTTPS、port付きURIを拒否する。
...
- DCRはportなし`http://127.0.0.1`の有効なCodex callbackだけを登録し、host/path/query不一致、`localhost`、IPv6、非loopback HTTP、HTTPS、wildcard、unsupported grant/response/auth methodを拒否する。`127.0.0.1` callbackのportだけは可変で通す。
